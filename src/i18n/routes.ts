...
//   - getLocalizedPath('de', 'donate') -> '/spenden'
//   - getLocalizedPath('en', 'donate') -> '/en/donate'
//   - getLocalizedPath('fr', 'donate') -> '/fr/dons'
//
// To switch a user's language while keeping them on the same page, we look up
// the page's route key from the current path, then resolve the new locale's
// slug for that key.
// -----------------------------------------------------------------------------
 ...
 * Slugs do NOT include locale prefix; that's added by buildPath().
...
export const routeSlugs: Record<RouteKey, Record<Locale, string>> = {
  ...
  maharishi: { de: 'maharishi',  en: 'maharishi', fr: 'maharishi' },
